...
# 
# rf.impcas <- randomForest(trainFea[,features], trainFea[,"casual"], ntree=100, importance=TRUE);
# imp <- importance(rf.impcas, type=1);
# featureImportance <- data.frame(Feature=row.names(imp), Importance=imp[,1]);
# sortedImportance.cas <- arrange(featureImportance, desc(Importance));
# acas<-sortedImportance.cas[1:20,1];
# 
# rf.impreg <- randomForest(trainFea[,features], trainFea[,"registered"], ntree=100, importance=TRUE);
# imp <- importance(rf.impreg, type=1);
# featureImportance <- data.frame(Feature=row.names(imp), Importance=imp[,1]);
# sortedImportance.reg <- arrange(featureImportance, desc(Importance));
# areg<-sortedImportance.reg[1:20,1];
...
for (i_year in unique(year(ymd_hms(test$datetime)))) {
  for (i_month in unique(month(ymd_hms(test$datetime)))) {
    ...
    for(h in 0:23){
      ...
      rf.casual <-randomForest(dataByHour[,features], dataByHour[,"casual"], ntree=100);
      rf.registered <- randomForest(dataByHour[,features],dataByHour[,"registered"], ntree=100);
      for (j in 1:nrow(testFea)){
        if ((testFea[j,"hour"] == h)) { 
          casual.prediction <- predict(rf.casual, testFea[j,features])
          registered.prediction <- predict(rf.registered, testFea[j,features])
    ...
    for (l in 1:nrow(testFea)){
      submission$count[submission.row] <- testFea$prediction[l]
